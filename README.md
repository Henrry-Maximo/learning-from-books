# 📚 Learning from Books

Repositório dedicado a centralizar minhas anotações, insights e exercícios práticos baseados nos livros de programação e computação que estou lendo.

## Livros

| Livro | Status | Tecnologias Utilizadas |
| :--- | :--- | :--- |
| **Entendendo Algoritmos** (Aditya Bhargava) | Lendo | JavaScript |
| **Aplicações Web Real-Time com NodeJS** (Caio Ribeiro Pereira) | Lendo | JavaScript |
| **Como Programar em C** (H.M. Deitel e P.J. Deitel) | Lendo | C |
| **Programação em Baixo Nível** (Igor Zhirkov) | Lendo | ... |
| **Código: a Vida Secreta dos Computadores** (Charles Petzold) | Lendo | ... |
| **O Programador Apaixonado** (Chad Fowler) | Lendo | ... |
| **A Prática da Programação** (Brian W. Kernighan) | Pausado | ... |
| **Fundamentos em Programação Assembly** (José Augusto N. G. Manzano) | Pausado | Assembly |
| **Entendendo Estruturas de Dados** (Marcello La Rocca) | Pausado | ... |
| **Algoritmos e Programação: Teoria e Prática** (Cristina Ferting e Marco Medina) |Pausado | ... |
| **Desenvolvimento Rápido de Sistemas** (Chris Gane) | Pausado | ... |

## Como está organizado?

A estrutura do repositório é dividida por livro dentro da pasta `/books`. Cada livro possui:
- `anotacoes.md`: Resumos conceituais, mapas mentais textuais e insights gerados durante a leitura.
- `/exercicios`: Códigos e desafios propostos pelo livro replicados na prática.

---
"O aprendizado contínuo é a única estratégia de longo prazo."
